...
func TestRuntimeRoutesNaturalLanguageRunAsJobSkillToJob(t *testing.T) {
	root := t.TempDir()
	storeRoot := t.TempDir()
	jobs := NewMemoryJobStore()
	manager := skills.NewSkillManager()
	docx := skills.NewSimpleSkill("docx", "Create Word documents. Triggers include: '生成文档', 'Docx 文档', 'Word 文档'", "docx prompt")
	docx.RunAsJob = true
	if err := manager.RegisterLoadedSkills([]*skills.SkillDefinition{docx}); err != nil {
	...
	runtime := NewRuntime(
		...
		func(scope Scope) Runner {
			...
			t.Fatalf("natural language run_as_job skill should not use the normal chat runner")
			return echoRunner{}
		},
	)
	runtime.SetJobStore(jobs)
	...
	content := "总结一下这个 txt 文件，然后生成一个 Docx 文档"
	decision := runtime.RouteChat(ChatRequest{UserID: "alice", SessionID: session.ID, Content: content})
	if !decision.RunAsJob || decision.JobType != "skill" || !decision.HideJobUserMessage {
		t.Fatalf("unexpected route decision: %#v", decision)
	}
	if decision.Content != "/docx "+content {
		t.Fatalf("unexpected routed content: %q", decision.Content)
	}
	sink := &collectSink{}
	if err := runtime.Chat(context.Background(), ChatRequest{UserID: "alice", SessionID: session.ID, Content: content}, sink); err != nil {
		t.Fatalf("chat: %v", err)
	}
	sink.mu.Lock()
	events := append([]Event(nil), sink.events...)
	...
	if routedJob.Type != "skill" || routedJob.Content != "/docx "+content {
		t.Fatalf("unexpected routed job: %#v", routedJob)
	}
	deadline := time.Now().Add(2 * time.Second)
	for {
	...
	for _, message := range updated.Messages {
		...
		if message.Role == "user" && !message.Hidden && strings.HasPrefix(strings.TrimSpace(message.Content), "/docx") {
			t.Fatalf("routed slash command should stay hidden: %#v", updated.Messages)
		}
	}
	if len(visibleUsers) != 1 || visibleUsers[0] != content {
		t.Fatalf("unexpected visible user messages: %#v", visibleUsers)
	}
}
