...
func TestProjectAPIAssociatesSessionsAndInjectsContext(t *testing.T) {
	root := t.TempDir()
	sessionStore := NewFileSessionStore(root)
	projectStore := NewFileProjectStore(root)
	runtime := NewRuntime(RuntimeConfig{DefaultWorkingDir: root, TurnTimeout: time.Minute}, sessionStore, nil, nil, func(Scope) Runner {
		return echoRunner{}
	})
	...
	createProjectReq := httptest.NewRequest(http.MethodPost, "/v1/projects", bytes.NewBufferString(`{"name":"Launch","description":"Q2 work","instructions":"Always mention launch constraints."}`))
	createProjectReq.Header.Set("X-User-ID", "alice")
	createProjectRec := httptest.NewRecorder()
	server.ServeHTTP(createProjectRec, createProjectReq)
	...
	if project.ID == "" || project.Name != "Launch" {
		t.Fatalf("unexpected project: %#v", project)
	}

	createSessionReq := httptest.NewRequest(http.MethodPost, "/v1/sessions", bytes.NewBufferString(`{"working_dir":"","project_id":"`+project.ID+`"}`))
	createSessionReq.Header.Set("X-User-ID", "alice")
	createSessionRec := httptest.NewRecorder()
	server.ServeHTTP(createSessionRec, createSessionReq)
	if createSessionRec.Code != http.StatusCreated {
		t.Fatalf("create session status = %d body=%s", createSessionRec.Code, createSessionRec.Body.String())
	}
	...
	listReq := httptest.NewRequest(http.MethodGet, "/v1/sessions?project_id="+url.QueryEscape(project.ID), nil)
	...
	if len(sessions) != 1 || sessions[0].ProjectID != project.ID {
		t.Fatalf("unexpected project sessions: %#v", sessions)
	}

	if err := runtime.Chat(context.Background(), ChatRequest{UserID: "alice", SessionID: session.ID, Content: "hello"}, &collectSink{}); err != nil {
		t.Fatalf("chat: %v", err)
	}
	saved, err := sessionStore.Get(context.Background(), "alice", session.ID)
	if err != nil {
		t.Fatalf("get saved session: %v", err)
	}
	var foundProjectContext bool
	for _, message := range saved.Messages {
		if message.Hidden && strings.Contains(message.Content, "Always mention launch constraints.") {
			foundProjectContext = true
			break
		}
	}
	if !foundProjectContext {
		t.Fatalf("project instructions were not injected: %#v", saved.Messages)
	}
}
