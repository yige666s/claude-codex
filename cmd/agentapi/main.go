...
func main() {
	...
	liveVADStartSensitivity := flag.String("live-vad-start-sensitivity", firstNonEmpty(os.Getenv("AGENT_API_LIVE_VAD_START_SENSITIVITY"), "START_SENSITIVITY_HIGH"), "Gemini Live VAD speech start sensitivity")
	liveVADEndSensitivity := flag.String("live-vad-end-sensitivity", firstNonEmpty(os.Getenv("AGENT_API_LIVE_VAD_END_SENSITIVITY"), "END_SENSITIVITY_HIGH"), "Gemini Live VAD speech end sensitivity")
	liveVADPrefixPadding := flag.Duration("live-vad-prefix-padding", envDuration("AGENT_API_LIVE_VAD_PREFIX_PADDING", 150*time.Millisecond), "Gemini Live VAD detected speech duration before confirming speech start")
	liveVADSilenceDuration := flag.Duration("live-vad-silence-duration", envDuration("AGENT_API_LIVE_VAD_SILENCE_DURATION", 500*time.Millisecond), "Gemini Live VAD silence duration before confirming speech end")
	...
	runtimeConfig := agentruntime.RuntimeConfig{
		...
		Live: agentruntime.LiveConfig{
			...
			LiveVADStartSensitivity:    *liveVADStartSensitivity,
			LiveVADEndSensitivity:      *liveVADEndSensitivity,
			LiveVADPrefixPadding:       *liveVADPrefixPadding,
			LiveVADSilenceDuration:     *liveVADSilenceDuration,
