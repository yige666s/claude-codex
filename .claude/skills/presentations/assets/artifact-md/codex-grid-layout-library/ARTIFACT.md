...
Run `scripts/snippets/create-presentation.mjs` to materialize the complete 80-slide reconstruction. Use `artifact-tool-compose/template-registry.json` to select individual exact modules. The modules deliberately contain no JSX.

# Validation

All 80 exact JavaScript modules must parse and render through artifact-tool. The final package must contain no Office file, original filename, transient evidence path, source screenshot, or raw extraction record.
