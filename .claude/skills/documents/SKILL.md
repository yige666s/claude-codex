...
This project runs the exported Codex office skills from `.claude/skills` inside
the AgentAPI container. Use the container-provided office runtime instead of
Codex's private workspace dependency loader:

- Python executable: `python3`
- DOCX helpers: bundled scripts in this skill folder, plus `python-docx`,
  `lxml`, `Pillow`, `openpyxl`, and related Python packages
- Rendering: `soffice`/LibreOffice and Poppler (`pdftoppm`, `pdfinfo`)
- Runtime smoke check:
  `python3 ../../skill-runtimes/office/check_office_runtime.py`

If the original instructions mention Codex workspace dependencies, interpret
that as the AgentAPI office runtime above.
...
When summarizing document work in Codex App, cite only the final delivered DOCX.

Use page citations when page numbers come from the latest rendered or inspected final document:
