...
- Preferred XLSX authoring and editing: `python3` with `openpyxl`
- JS authoring option: `node` with global `exceljs`
- CSV/TSV processing: Python standard library, `pandas`, or Node as needed
- Visual/export checks: LibreOffice (`soffice`) when available
- Runtime smoke check:
...
implementation to `openpyxl` or `exceljs` while preserving the same workbook
quality gates: typed values, formulas instead of hardcoded derived outputs,
readable formatting, and final artifact registration.
